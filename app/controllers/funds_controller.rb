class FundsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to funds_path, status: :see_other, notice: "Fund was successfully destroyed." }
  ...
  private
    ...
    def set_fund
      @fund = Fund.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def fund_params
      params.expect(fund: [ :logo, :name, :price, :quantity, :description, :start_time, :end_time, :official_web, :user_id ])
    end
end
