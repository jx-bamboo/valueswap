...
class ExchangesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create exchange" do
    assert_difference("Exchange.count") do
      post exchanges_url, params: { exchange: { logo: @exchange.logo, name: @exchange.name, status: @exchange.status } }
    end
  ...
  test "should update exchange" do
    patch exchange_url(@exchange), params: { exchange: { logo: @exchange.logo, name: @exchange.name, status: @exchange.status } }
